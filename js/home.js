...
function renderHeroFromSettings() {
  const heroSlider = document.querySelector("[data-hero-slider]");
  if (!heroSlider) return;

  const settings = window.SITE_SETTINGS;
  const heading = heroSlider.querySelector(".hero-overlay .eyebrow");
  if (heading && settings?.hero_heading) {
    heading.textContent = settings.hero_heading;
  }

  const slides = settings?.hero_slides;
  if (Array.isArray(slides) && slides.length) {
    const track = heroSlider.querySelector(".slider-track");
    const dotsContainer = heroSlider.querySelector("[data-slider-dots]");
